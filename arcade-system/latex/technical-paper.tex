...
% رفع خطای ارتفاع هدر (افزایش به 30pt)
...
\begin{document}
	...
	\begin{abstract}
		...
		در این مقاله به تحلیل معماری، تصمیمات طراحی، جزئیات پیاده‌سازی، چالش‌ها، و راهکارهای آینده پرداخته می‌شود.
...
\begin{figure}[H]
	...
	\begin{latin}
		\begin{verbatim}
			...
			v
			+---------------------------------------------+
			|      Python HTTP Server (CGI Handler)       |
			+--------------+------------------------------+
			| Execute CGI
			v
			+---------------------------------------------+
			...
			|  +---------+ +---------+ +----------+       |
			|  | users   | |sessions | |admin_logs|       |
			|  | .json   | | .json   | |  .json   |       |
			|  +---------+ +---------+ +----------+       |
			+---------------------------------------------+
		\end{verbatim}
	\end{latin}
	\caption{نمای کلی معماری سیستم بازی‌لند}
\end{figure}
	...
	\subsection{تست دستی}
	تمام بازی‌ها به صورت دستی روی مرورگرهای زیر تست شده‌اند:
	...
	\subsubsection{امنیت}
	\begin{itemize}
		\item \lr{Hash} کردن رمزهای عبور با \lr{bcrypt}
		\item پیاده‌سازی \lr{HTTPS}
		\item \lr{CSRF protection}
		\item \lr{Rate limiting}
		\item \lr{Input validation} بهتر
	\end{itemize}
	...
	از تمام منابع و ابزارهای \lr{Open Source} که در این پروژه استفاده شده، تشکر می‌کنم:
	
	\begin{itemize}
		\item \lr{Font Awesome} برای آیکون‌ها
		\item \lr{Particles.js} برای افکت پس‌زمینه
		\item \lr{Vazirmatn Font} برای فونت فارسی
		\item \lr{Python HTTP Server} برای سرویس \lr{CGI}
	\end{itemize}
	...
	% بخش Makefile که ارور داشت حذف شد
