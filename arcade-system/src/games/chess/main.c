/**
 * @file src/games/chess/main.c
 * @project arcade-system
 * @author Alireza Zarebidoki
 * @date_created 2024
 * @date_modified 2024
 * @brief Main CGI handler for modified chess game with rendering and move processing.
 *
 * @details\n * این فایل CGI اصلی شطرنج است که رندر تخته، دریافت کلیک و مدیریت حرکات را انجام می‌دهد.
 * تمام منطق رندر خانه‌های تخته، نمایش مهره‌ها، و تولید لینک‌های حرکات در اینجا است.
 *
 * @responsibilities
 * - رندر تخته بازی 8×8 با خانه‌های رنگی (طوسی و نقره‌ای)
 * - نمایش مهره‌های بازی شامل مهره‌های سفارشی
 * - هایلایت خانه‌های انتخاب شده و حرکات معتبر
 * - نمایش تهدیدات کیش برای هر دو شاه
 * - تولید لینک‌های کلیک‌پذیر برای هر خانه
 * - سریال‌سازی state بازی برای انتقال از طریق URL
 * - نمایش آیکون‌های Font Awesome برای مهره‌ها
 *
 * @interactions
 * - دریافت state بازی و موقعیت کلیک از URL parameters
 * - استفاده از chess.h برای تعریف‌های داده‌ای
 * - فراخوانی توابع logic.c برای بررسی حرکات معتبر و کیش
 * - فراخوانی توابع state.c برای مدیریت state
 * - استفاده از c_utils برای HTML wrapper
 *
 * @notes
 * - رنگ خانه‌ها بر اساس موقعیت (row+col) مشخص می‌شود
 * - هایلایت برای انتخاب و حرکات (سبز عادی)
 * - نمایش کیش برای شاه‌های در خطر (قرمز)
 * - آیکون‌های Font Awesome برای نمایش مهره‌های سفارشی
 * - تمام حرکات از طریق URL link انجام می‌شود (stateless)
 */
