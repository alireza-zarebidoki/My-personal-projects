 ...
 * @date_created 2024
 * @date_modified 2024
 * @brief Main client-side application controller for arcade cabinet interface.
 *
 * @details
 * این فایل را من خودم از صفر نوشتم تا تمام منطق سمت کلاینت کابینت آرکید
 * را مدیریت کند. این فایل قلب تپنده رابط کاربری است و تمام تعاملات صفحه
 * را کنترل می‌کند.
 *
 * @responsibilities
 * - مدیریت منوی چرخشی (carousel) بازی‌ها و پیمایش میان آن‌ها
 * - کنترل احراز هویت و رابط (login/register/logout)
 * - مدیریت سکه‌ها و اقتصاد بازی و موجودی‌های کاربری
 * - بارگذاری و اجرای بازی‌ها در iframe و مدیریت state
 * - مدیریت پنل کاربری و پنل ادمین برای نگارش اطلاعات
 * - رابط راهنما (help modal) با اطلاعات جامع بازی‌ها
 * - انیمیشن ذرات پس‌زمینه و افکت‌های بصری
 *
 * @interactions
 * - ارتباط با auth.js برای عملیات احراز هویت و مدیریت سکه
 * - بارگذاری particles.js برای انیمیشن پس‌زمینه
 * - تعامل با صفحه HTML و عناصر DOM مختلف
 * - ارسال درخواست‌های بازی به سرورهای CGI
 *
 * @notes
 * - این فایل با auth.js ارتباط نزدیک دارد و قبل از auth.js لود می‌شود
 * - تغییرات در اینجا باید با سرور CGI هماهنگ باشد
 * - قیمت بازی‌ها باید با سرور همگام بماند
 * - particles.js اختیاری است؛ اگر لود نشود صفحه بدون انیمیشن کار می‌کند
 */
